...
static void __attribute__((noinline)) shake128_squeeze_secret(uint16_t s[SABER_N], shake128incctx *ctx)
{
	...
	{
		s[4*j] = (((shake_out[j]) & 0x03) ^ 2) - 2; 
		s[4*j+1] = (((shake_out[j] >> 2) & 0x03)  ^ 2) - 2;
		s[4*j+2] = (((shake_out[j] >> 4) & 0x03)  ^ 2) - 2;
		s[4*j+3] = (((shake_out[j] >> 6) & 0x03)  ^ 2) - 2;
	}
}
...
void GenSecret(uint16_t s[SABER_L][SABER_N], uint8_t seed_s[SABER_NOISE_SEEDBYTES], int keypair)
{
    ...
    shake128(buf, SABER_L * SABER_POLYCOINBYTES, seed_s, SABER_NOISE_SEEDBYTES);
...
{
    size_t i, j;
    uint16_t v[SABER_SHARES][SABER_N], vtemp[SABER_N][SABER_SHARES];//,vtemp1[SABER_N][SABER_SHARES];
    uint16_t bp[SABER_N];
    uint16_t (*cm) = bp;
